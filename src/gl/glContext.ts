...
function getSupportedFormat(
  ...
  // First check whether the requested format can be used as a render target.
  if (!supportRenderTextureFormat(gl, internalFormat, format, type)) {
    // Fallback is only needed for WebGL2 16F-like formats.
    if ("RGBA16F" in gl) {
      const gl2 = gl as WebGL2RenderingContext;
    ...
    // Return null for WebGL1 or when no fallback format is available.
    return null;
  }

  // The requested format works.
  return {
    internalFormat,
    format,
  };
